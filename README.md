...
```bash
# コンテンツを配置するディレクトリ（デフォルト: my-blog-contents）
CONTENT_DIR=my-blog-contents
# 記事ルートディレクトリ（デフォルト: external-posts）
POSTS_ROOT_DIR=external-posts
```
...
ビルド前にコンテンツリポジトリを `my-blog-contents/` に取得します。Cloudflare Pages なら Build Command に `./scripts/fetch-content-and-build.sh` を設定してください。

```bash
git clone https://github.com/your_user/your_content_repo "$CONTENT_DIR"
bun run build
```
...
git clone git@github.com:your_user/your_content_repo "$CONTENT_DIR"
bun run build
```
...
| フィールド | 型                                    | デフォルト値 | 説明                                     | 例         |
| ---------- | ------------------------------------- | ------------ | ---------------------------------------- | ---------- |
| `access`   | `"public" \| "unlisted" \| "private"` | `"private"`  | アクセス制限。`"unlisted"`は直リンクのみ | `"public"` |
